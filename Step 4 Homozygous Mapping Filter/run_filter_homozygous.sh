...
SRC= #Location to scripts in MOLBAR_HOME

echo "Write counts to *.tsv ..."

python3 ${SRC}/filter_homozygous.py Combined.model.tsv    >    Combined.min_and_fold.tsv

echo "Write genes to *.genes_pass_filter ..."
echo "Filter for min fold change 5"
# Use c to skip the header line

cat    Combined.min_and_fold.tsv | awk '{if ((c++ >0) && ($4>=5)) print $1;}' >    Combined.genes_pass_filter
