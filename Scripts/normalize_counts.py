...
class NormalizeCounts(object):
    ...
    def parse_line(self,dataline):
        ...
        f=0
        while f <= 8:   # process fields 1 through 6
            if f==0:
    ...
    def apply_normal(self, array_counts):
        ...
        for r in range(len(array_counts)):
            ...
            norm1_5 += fields[1]+fields[5]
            norm2_6 += fields[2]+fields[6]
            norm3_7 += fields[3]+fields[7]
            norm4_8 += fields[4]+fields[8]
        total_reads =  norm1_5 + norm2_6 +  norm3_7 + norm4_8
        average_reads = total_reads/4
        print("total_reads,%d"%total_reads)
        print("average_reads,%f"%average_reads)
        print("normalize_BR1,%f"%(average_reads/norm1_5))
        print("normalize_BR2,%f"%(average_reads/norm2_6))
        print("normalize_BR3,%f"%(average_reads/norm3_7))
        print("normalize_BR4,%f"%(average_reads/norm4_8))
        for r in range(len(array_counts)):
            line = array_counts[r]
            fields = self.parse_line(line)
            fields[1] = self.float_to_count(fields[1]*average_reads/norm1_5)
            fields[2] = self.float_to_count(fields[2]*average_reads/norm2_6)
            fields[3] = self.float_to_count(fields[3]*average_reads/norm3_7)
            fields[4] = self.float_to_count(fields[4]*average_reads/norm4_8)
            fields[5] = self.float_to_count(fields[5]*average_reads/norm1_5)
            fields[6] = self.float_to_count(fields[6]*average_reads/norm2_6)
            fields[7] = self.float_to_count(fields[7]*average_reads/norm3_7)
            fields[8] = self.float_to_count(fields[8]*average_reads/norm4_8)
